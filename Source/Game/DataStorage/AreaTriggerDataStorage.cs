...
namespace Game.DataStorage
{
    public class AreaTriggerDataStorage : Singleton<AreaTriggerDataStorage>
    {
        ...
        public void LoadAreaTriggerTemplates()
        {
            ...
            {
                //                                                       0         1             2            3           4
                using var templateActions = DB.World.Query("SELECT AreaTriggerId, IsServerSide, ActionType, ActionParam, TargetType FROM `areatrigger_template_actions`");
            ...
            {
                //                                           0                              1    2         3         4               5
                using var vertices = DB.World.Query("SELECT AreaTriggerCreatePropertiesId, Idx, VerticeX, VerticeY, VerticeTargetX, VerticeTargetY FROM `areatrigger_create_properties_polygon_vertex` ORDER BY `AreaTriggerCreatePropertiesId`, `Idx`");
                if (!vertices.IsEmpty())
                {
                    do
                    {
            ...
            {
                //                                         0                              1  2  3
                using var splines = DB.World.Query("SELECT AreaTriggerCreatePropertiesId, X, Y, Z FROM `areatrigger_create_properties_spline_point` ORDER BY `AreaTriggerCreatePropertiesId`, `Idx`");
                if (!splines.IsEmpty())
                {
                    do
                    {
            ...
            {
                //                                            0   1             2
                using var templates = DB.World.Query("SELECT Id, IsServerSide, Flags FROM `areatrigger_template`");
...

            {
                ...
                using var areatriggerCreateProperties = DB.World.Query("SELECT Id, AreaTriggerId, MoveCurveId, ScaleCurveId, MorphCurveId, FacingCurveId, AnimId, AnimKitId, DecalPropertiesId, TimeToTarget, TimeToTargetScale, " +
            ...
            {
                //                                                       0                               1           2             3                4             5        6                 7
                using var circularMovementInfos = DB.World.Query("SELECT AreaTriggerCreatePropertiesId, StartDelay, CircleRadius, BlendFromRadius, InitialAngle, ZOffset, CounterClockwise, CanLoop FROM `areatrigger_create_properties_orbit`");
        ...
        public void LoadAreaTriggerSpawns()
        {
            ...
            using var result = DB.World.Query("SELECT SpawnId, AreaTriggerId, IsServerSide, MapId, SpawnDifficulties, PosX, PosY, PosZ, Orientation, PhaseUseFlags, PhaseId, PhaseGroup, " +
