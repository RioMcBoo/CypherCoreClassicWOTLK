...
namespace Game.DungeonFinding
{
    public class LFGQueue
    {
        ...
        public void AddToQueue(ObjectGuid guid, bool reAdd = false)
        {
            if (!QueueDataStore.ContainsKey(guid))
            {
                Log.outError(LogFilter.Lfg, 
                    $"AddToQueue: Queue data not found for [{guid}]");
                return;
            }
        ...
        public void AddQueueData(ObjectGuid guid, ServerTime joinTime, List<int> dungeons, Dictionary<ObjectGuid, LfgRoles> rolesMap)
        {
        ...
        public void UpdateWaitTimeAvg(TimeSpan waitTime, int dungeonId)
        {
            LfgWaitTime wt = waitTimesAvgStore[dungeonId];
            uint old_number = wt.number++;
            wt.time = (Seconds)((wt.time * old_number + (Seconds)waitTime) / wt.number);
        }

        public void UpdateWaitTimeTank(TimeSpan waitTime, int dungeonId)
        {
        ...
        public void UpdateWaitTimeHealer(TimeSpan waitTime, int dungeonId)
        {
        ...
        public void UpdateWaitTimeDps(TimeSpan waitTime, int dungeonId)
        {
        ...
        void RemoveFromCompatibles(ObjectGuid guid)
        {
            ...
            Log.outDebug(LogFilter.Lfg, $"RemoveFromCompatibles: Removing [{guid}]");
        ...
        public byte FindGroups()
        {
            ...
            while (!newToQueueStore.Empty())
            {
                ...
                Log.outDebug(LogFilter.Lfg, 
                    $"FindGroups: checking [{frontguid}] newToQueue({newToQueueStore.Count}), " +
                    $"currentQueue({currentQueueStore.Count})");
        ...
        LfgCompatibility FindNewGroups(List<ObjectGuid> check, List<ObjectGuid> all)
        {
            ...
            Log.outDebug(LogFilter.Lfg, 
                $"FindNewGroup: ({strGuids}): {GetCompatibleString(compatibles)} - all({ConcatenateGuids(all)})");
            ...
            {
                Log.outDebug(LogFilter.Lfg, 
                    $"FindNewGroup: ({strGuids}) compatibles (cached) changed from bad states to match");
        ...
        LfgCompatibility CheckCompatibility(List<ObjectGuid> check)
        {
            ...
            {
                Log.outDebug(LogFilter.Lfg, 
                    $"CheckCompatibility: ({strGuids}): Size wrong - Not compatibles");
            ...
            if (check.Count > 2)
            {
                ...
                {
                    Log.outDebug(LogFilter.Lfg, 
                        $"CheckCompatibility: ({strGuids}) child {ConcatenateGuids(check)} not compatibles");
            ...
            foreach (var guid in check)
            {
                ...
                {
                    Log.outError(LogFilter.Lfg, 
                        $"CheckCompatibility: [{guid}] is not queued but listed as queued!");
            ...
            {
                Log.outDebug(LogFilter.Lfg, 
                    $"CheckCompatibility: ({strGuids}) sigle group. Compatibles");
            ...
            {
                Log.outDebug(LogFilter.Lfg, 
                    $"CheckCompatibility: ({strGuids}) More than one Lfggroup ({numLfgGroups})");
            ...
            {
                Log.outDebug(LogFilter.Lfg, 
                    $"CheckCompatibility: ({strGuids}) Too much players ({numPlayers})");
            ...
            if (check.Count > 1)
            {
                foreach (var it in check)
                {
                    ...
                    foreach (var rolePair in roles)
                    {
                        ...
                        foreach (var _player in proposalRoles)
                        {
                            ...
                            {
                                Log.outError(LogFilter.Lfg,
                                    $"CheckCompatibility: ERROR! Player multiple times in queue! [{rolePair.Key}]");
                            }
                ...
                {
                    Log.outDebug(LogFilter.Lfg, 
                        $"CheckCompatibility: ({strGuids}) not compatible, " +
                        $"{playersize} players are ignoring each other");
                ...
                if (!Global.LFGMgr.CheckGroupRoles(proposalRoles))
                {
                    ...
                    foreach (var it in debugRoles)
                        o.AppendFormat($", {it.Key}: {GetRolesString(it.Value)}");

                    Log.outDebug(LogFilter.Lfg, $"CheckCompatibility: ({strGuids}) Roles not compatible{o}");
                ...
                o = new StringBuilder();
                o.AppendFormat($", {itguid}: ({Global.LFGMgr.ConcatenateDungeons(proposalDungeons)})");
                foreach (var guid in check)
                {
                    if (guid == itguid)
                        continue;
                ...
                {
                    Log.outDebug(LogFilter.Lfg, $"CheckCompatibility: ({strGuids}) No compatible dungeons{o}");
            ...
            {
                Log.outDebug(LogFilter.Lfg, 
                    $"CheckCompatibility: ({strGuids}) Compatibles but not enough players({numPlayers})");
            ...
            {
                Log.outDebug(LogFilter.Lfg, 
                    $"CheckCompatibility: ({strGuids}) Group MATCH but can't create proposal!");
            ...
            proposal.cancelTime = LoopTime.ServerTime + SharedConst.LFGTimeProposal;
            ...
            foreach (var rolePair in proposalRoles)
            {
                ...
                {
                    if (!leader || proposal.leader.IsEmpty() || RandomHelper.IRand(0, 1) == 1)
                        proposal.leader = rolePair.Key;
                    leader = true;
                }
                else if (!leader && (proposal.leader.IsEmpty() || RandomHelper.IRand(0, 1) == 1))
                    proposal.leader = rolePair.Key;
            ...
            Log.outDebug(LogFilter.Lfg, 
                $"CheckCompatibility: ({strGuids}) MATCH! Group formed");

            SetCompatibles(strGuids, LfgCompatibility.Match);
            return LfgCompatibility.Match;
        }

        public void UpdateQueueTimers(byte queueId, ServerTime currTime)
        {
            ...
            foreach (var itQueue in QueueDataStore)
            {
                ...
                Seconds queuedTime = (Seconds)(currTime - queueinfo.joinTime);
        ...
        public ServerTime GetJoinTime(ObjectGuid guid)
        {
            var queueData = QueueDataStore.LookupByKey(guid);
            if (queueData != null)
        ...
        {
            Log.outDebug(LogFilter.Lfg, $"FindBestCompatibleInQueue: {guid}");
        ...
        public void UpdateBestCompatibleInQueue(ObjectGuid guid, LfgQueueData queueData, string key, Dictionary<ObjectGuid, LfgRoles> roles)
        {
            ...
            Log.outDebug(LogFilter.Lfg, 
                ...
                $"Changed ({queueData.bestCompatible}) to ({key}) " +
                $"as best compatible group for {guid}");
    ...
    public class LfgQueueData
    {
        public LfgQueueData()
        {
            joinTime = LoopTime.ServerTime;
            tanks = SharedConst.LFGTanksNeeded;
            healers = SharedConst.LFGHealersNeeded;
            dps = SharedConst.LFGDPSNeeded;
        }

        public LfgQueueData(ServerTime _joinTime, List<int> _dungeons, Dictionary<ObjectGuid, LfgRoles> _roles)
        {
            joinTime = _joinTime;
            tanks = SharedConst.LFGTanksNeeded;
            healers = SharedConst.LFGHealersNeeded;
