...
namespace Game.Maps
{
    ...
    public class InstanceLockManager : Singleton<InstanceLockManager>
    {
        ...
        public void Load()
        {
            ...
            //                                                  0     1      2       3           4           5     6                        7           8
            using var lockResult = DB.Characters.Query("SELECT guid, mapId, lockId, instanceId, difficulty, data, completedEncountersMask, expiryTime, extended FROM character_instance_lock");
            if (!result.IsEmpty())
            {
                do
                {
